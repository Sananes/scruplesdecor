...
class Zebra_Image
{
	...
	function resize($width = 0, $height = 0, $method = ZEBRA_IMAGE_CROP_CENTER, $background_color = '#FFFFFF')
	{
		...
		if ($this->_create_from_source()) {
			...
			if ($this->preserve_aspect_ratio || isset($auto_preserve_aspect_ratio) || $method == ZEBRA_IMAGE_BOX_FIT) {
				
				# start: modified by Arlind Nushi
				if($method == ZEBRA_IMAGE_BOX_FIT)
				{
					$original_size 		= getimagesize($this->source_path);
					...
					if($width && $height)
					{
						$target_identifier 	= $this->_prepare_image($width, $height, '#FFF');
						...
						# If image is smaller than box
						$extra_scale_pct = 1.1;
						
						
						if($o_width * $extra_scale_pct < $width && $o_height * $extra_scale_pct < $height)
						{
							$dst_w = $o_width;
							$dst_h = $o_height;
							
							$dst_x = intval($width / 2 - $o_width / 2);
							$dst_y = intval($height / 2 - $o_height / 2);
							
							imagecopyresampled($target_identifier, $this->source_identifier, $dst_x, $dst_y, $src_x, $src_y, $dst_w, $dst_h, $src_w, $src_h);
						...
						else
						{
							...
							{
								$dst_w = $width;
								$dst_h = _rbW($o_width, $o_height, $width);
								
								if($dst_h < $height)
								{
									$dst_w = _rbH($o_width, $o_height, $height);
									$dst_h = $height;
								}
								$dst_x = -intval(($dst_w - $width) / 2);
								$dst_y = -intval(($dst_h - $height) / 2);
							}
							else
							{
								$dst_w = _rbH($o_width, $o_height, $height);
				...
				// if both width and height are given and ZEBRA_IMAGE_BOXED or ZEBRA_IMAGE_NOT_BOXED methods are to be used
					...
					if (round($horizontal_aspect_ratio * $this->source_height < $height)) {
						...
						$target_height = ceil($horizontal_aspect_ratio * $this->source_height);
					...
					// otherwise
						...
						$target_width = ceil($vertical_aspect_ratio * $this->source_width);
			...
			if (
				...
				// it needs to be preserved and method is ZEBRA_IMAGE_BOXED or ZEBRA_IMAGE_NOT_BOXED
					...
					$target_w = $func_args[0];
					
					if($target_w > $target_width)
					{
						$tw_ratio = $target_w / $target_width;
						
						$target_width = $target_w;
						$target_height = $tw_ratio * $target_height;
			...
			elseif($method == ZEBRA_IMAGE_BOXED)
			{
				$original_size = getimagesize($this->source_path);
				...
				{
					$dst_x = 0;
					$dst_y = intval(($height - $o_height) / 2);
...
# start: modified by Arlind Nushi
function _rbH($w, $h, $nh){ return intval($w * ($nh / $h)); }
function _rbW($w, $h, $nw){ return intval($h * ($nw / $w)); }
# end: modified by Arlind Nushi