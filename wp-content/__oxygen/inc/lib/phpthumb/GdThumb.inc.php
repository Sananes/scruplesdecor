...
class GdThumb extends ThumbBase
{
	 ...
	 * @return GdThumb
	 */
	public function resize ($maxWidth = 0, $maxHeight = 0, $fixedCanvas = false, $bgColor = '#EEEEEE')
	{
		...
		if (function_exists('imagecreatetruecolor'))
		{
			...
			if($fixedCanvas)
			{
				...
				list($r, $g, $b) = ColorsOfImage::HexToRGB($bgColor);
				imagefill($this->workingImage, 0, 0, imagecolorallocate($this->workingImage, $r, $g, $b));
			}
			else
		...
		if($fixedCanvas)
		{
			$dist_x =  absint(($maxWidth - $this->newDimensions['newWidth'])/2);
			$dist_y =  absint(($maxHeight - $this->newDimensions['newHeight'])/2);
	 ...
	 * @return GdThumb
	 */
	public function adaptiveResize ($width, $height, $crop_pos = 'cc')
	{
		// make sure our arguments are valid
		...
		switch($crop_pos)
		{
			...
			case "b":	
			case "bottom":
				$cropY = $this->currentDimensions['height'] - $cropHeight;
				break;
		}
		# end: modified by Arlind Nushi
	...
	public function setOptions ($options = array())
	{
		...
		if (sizeof($this->options) == 0)
		{
			...
			(
				'resizeUp'				=> false,
				'jpegQuality'			=> 90, #get_data('image_resizer_jpeg_quality') ? get_data('image_resizer_jpeg_quality') : 100,
