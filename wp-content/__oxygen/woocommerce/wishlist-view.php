...
<form id="yith-wcwl-form" class="laborator-woocommerce shop" action="<?php echo esc_url( YITH_WCWL()->get_wishlist_url( 'view' . ( $wishlist_meta['is_default'] != 1 ? '/' . $wishlist_meta['wishlist_token'] : '' ) ) ) ?>" method="post" class="woocommerce">
    ...
    ?>
    	...
    	<div class="row">
			<div class="col-lg-12">
				<div class="white-block block-pad top-title">
			        ...
			        <?php if( $wishlist_meta['is_default'] != 1 && $is_user_owner ): ?>
			            <div class="hidden-title-form">
			                <input type="text" value="<?php echo $page_title ?>" class="form-control" name="wishlist_name"/>
			                <button class="btn button btn-primary">
			                    <?php echo apply_filters( 'yith_wcwl_save_wishlist_title_icon', '<i class="fa fa-check"></i>' )?>
			                    <?php _e( 'Save', 'yit' )?>
			                </button>
			                <a class="hide-title-form btn button btn-secondary">
