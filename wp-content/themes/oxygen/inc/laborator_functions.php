...
function share_story_network_link($network, $id, $simptips = true)
{
	...
	$networks = array(
		'fb' => array(
			'url'		=> '//www.facebook.com/sharer.php?m2w&s=100&p&#91;url&#93;=' . get_permalink() . '&p&#91;title&#93;=' . urlencode( get_the_title() ),
			'tooltip'	=> __('Share on Facebook', 'oxygen'),
			'icon'		=> 'facebook'
		),

		'tw' => array(
			'url'		=> '//twitter.com/home?status=' . urlencode( get_the_title() ) . ' ' . get_permalink(),
			'tooltip'	=> __('Share on Twitter', 'oxygen'),
			'icon'		 => 'twitter'
		),

		'gp' => array(
			'url'		=> '//plus.google.com/share?url=' . get_permalink(),
			'tooltip'	=> __('Share on Google+', 'oxygen'),
			'icon'		 => 'gplus'
		),

		'tlr' => array(
			'url'		=> '//www.tumblr.com/share/link?url=' . get_permalink() . '&name=' . urlencode( get_the_title() ) . '&description=' . urlencode( get_the_excerpt() ),
			'tooltip'	=> __('Share on Tumblr', 'oxygen'),
			'icon'		 => 'tumblr'
		),

		'lin' => array(
			'url'		=> '//linkedin.com/shareArticle?mini=true&amp;url=' . get_permalink() . '&amp;title=' . urlencode( get_the_title() ),
			'tooltip'	=> __('Share on LinkedIn', 'oxygen'),
			'icon'		 => 'linkedin'
		),

		'pi' => array(
			'url'		=> '//pinterest.com/pin/create/button/?url=' . get_permalink() . '&amp;description=' . urlencode( get_the_title() ) . '&amp;' . ($id ? ('media=' . wp_get_attachment_url( get_post_thumbnail_id($id) )) : ''),
			'tooltip'	=> __('Share on Pinterest', 'oxygen'),
			'icon'	 	 => 'pinterest'
		),

		'vk' => array(
			'url'		=> '//vkontakte.ru/share.php?url=' . get_permalink(),
