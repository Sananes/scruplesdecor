...
$rules_of_commenting = __('Please do not use offensive vocabulary.', TD);

$form_args = array(
	'title_reply' 			=> '<h3 class="title">' . (have_comments() ? __('Reply to Article', TD) : __('Share your thoughts', TD)) . ($rules_of_commenting ? "<small>{$rules_of_commenting}</small>" : '') . '</h3>',
	'title_reply_to' 		=> '<h3 class="title">' . __('Leave a Reply to %s', TD) . '</h3>',
	...
	'comment_field'			=> '<textarea id="comment" name="comment" class="form-control autogrow" placeholder="' . __('Message:', TD) . '" rows="3" aria-required="true"></textarea>'
);
...
<div class="comments">
	...
	<h3 class="title">
		<?php
			printf( _n( '1 Comment', '%1$s Comments', get_comments_number(), TD ), number_format_i18n( get_comments_number() ), get_the_title() );
		?>
	</h3>
