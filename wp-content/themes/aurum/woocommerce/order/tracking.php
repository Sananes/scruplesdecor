...
$order_status_text = sprintf( __( 'Order <mark>%s</mark> which was made <mark>%s</mark> has the status &ldquo;<mark>%s</mark>&rdquo;', 'woocommerce' ), $order->get_order_number(), human_time_diff( strtotime( $order->order_date ), current_time( 'timestamp' ) ) . ' ' . __( 'ago', 'woocommerce' ), wc_get_order_status_name( $order->get_status() ) );
