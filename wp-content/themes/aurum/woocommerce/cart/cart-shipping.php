...
?>
<tr class="shipping">
	<th<?php echo get_option( 'woocommerce_shipping_method_format' ) === 'select' ? ' valign="middle"' : ''; ?>><?php
		...
		<?php if ( ! empty( $available_methods ) ) : ?>
			...
			<?php else : ?>
				...
				<ul id="shipping_method">
					<?php foreach ( $available_methods as $method ) : ?>
						<li>
							<input type="radio" name="shipping_method[<?php echo $index; ?>]" data-index="<?php echo $index; ?>" id="shipping_method_<?php echo $index; ?>_<?php echo sanitize_title( $method->id ); ?>" value="<?php echo esc_attr( $method->id ); ?>" <?php checked( $method->id, $chosen_method ); ?> class="shipping_method replaced-radio-buttons" />
