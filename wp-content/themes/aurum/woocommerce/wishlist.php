...
<form id="yith-wcwl-form" action="<?php echo esc_url( $yith_wcwl->get_wishlist_url() ) ?>" method="post">
    ...
    if( !empty( $wishlist_title ) )
    {
	    ...
	    <div class="page-title">
			<h2>
				<?php echo apply_filters( 'yith_wcwl_wishlist_title', $wishlist_title ); ?>
				<small><?php _e('Your favorite list of products', TD); ?></small>
			</h2>
		</div>
	    <?php
    ...
    ?>
    <table class="shop_table cart wishlist_table view-cart" cellspacing="0">
        <thead>
	        <thead>
				<tr>
					<th class="product-name"><?php _e( 'Product', 'yith' ); ?></th>
					<th></th>
					<?php if( get_option( 'yith_wcwl_stock_show' ) == 'yes' ) : ?><th class="product-quantity"><?php _e( 'Stock Status', 'yith' ); ?></th><?php endif; ?>
					<?php if( get_option( 'yith_wcwl_add_to_cart_show' ) == 'yes' ) : ?><th class="product-subtotal"><?php _e( 'Add to Cart', 'yith' ); ?></th><?php endif; ?>
				</tr>
			</thead>
        </thead>
        <!--
...
-->
        ...
        if( count( $wishlist ) > 0 ) :
            foreach( $wishlist as $values ) :
                ...
                if( $product_obj !== false && $product_obj->exists() ) : ?>
                    <tr id="yith-wcwl-row-<?php echo $values['ID'] ?>">
                        ...
                        <td class="item-image">
	                        <a href="javascript:void(0)" onclick="<?php echo $remove_wishlist ?>" class="remove remove-item" title="<?php _e( 'Remove this product', 'yit' ) ?>"></a>

							<a href="<?php echo esc_url( get_permalink( apply_filters( 'woocommerce_in_cart_product', $values['prod_id'] ) ) ) ?>">
								<?php echo $product_obj->get_image('shop-thumb-2') ?>
							</a>
                        </td>
            ...
            <tr>
                <td colspan="4" class="wishlist-empty"><?php _e( 'No products were added to the wishlist', 'yit' ) ?></td>
            </tr>
        <?php
        endif;
