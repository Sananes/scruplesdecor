...
class WC_CSV_Parser {
	...
	public function parse_product( $item, $merge_empty_cells = 0 ) {
		...
		if ( $merging ) {
			...
			$WC_CSV_Product_Import->log->add( 'csv-import', sprintf( __('> Row %s - preparing for merge.', 'woocommerce-product-csv-import-suite'), $this->row ) );

			// Required fields
			if ( ! $post_id && empty( $item['sku'] ) ) {

				$WC_CSV_Product_Import->log->add( 'csv-import', __( '> > Cannot merge without id or sku. Importing instead.', 'woocommerce-product-csv-import-suite') );
				...
				if ( ! $post_id ) {
					...
					if ( ! $found_product_id ) {
						$WC_CSV_Product_Import->log->add( 'csv-import', sprintf(__( '> > Skipped. Cannot find product with sku %s. Importing instead.', 'woocommerce-product-csv-import-suite'), $item['sku']) );
						...
						$WC_CSV_Product_Import->log->add( 'csv-import', sprintf(__( '> > Found product with ID %s.', 'woocommerce-product-csv-import-suite'), $post_id) );
		...
		if ( ! $merging ) {
			...
			$WC_CSV_Product_Import->log->add( 'csv-import', sprintf( __('> Row %s - preparing for import.', 'woocommerce-product-csv-import-suite'), $this->row ) );

			// Required fields
			if ( $this->post_type == 'product' && empty( $item['post_title'] ) ) {
				$WC_CSV_Product_Import->log->add( 'csv-import', __( '> > Skipped. No post_title set for new product.', 'woocommerce-product-csv-import-suite') );
		...
		foreach ( $item as $key => $value ) {
			...
			elseif ( strstr( $key, 'tax:' ) ) {
				...
				if ( ! taxonomy_exists( $taxonomy ) ) {
					$WC_CSV_Product_Import->log->add( 'csv-import', sprintf( __('> > Skipping taxonomy "%s" - it does not exist.', 'woocommerce-product-csv-import-suite'), $taxonomy ) );
					continue;
				}
				...
				if ( $taxonomy == 'product_type' ) {
					...
					if ( ! array_key_exists( $term, $this->allowed_product_types ) ) {
						$WC_CSV_Product_Import->log->add( 'csv-import', sprintf( __('> > > Product type "%s" not allowed - using simple.', 'woocommerce-product-csv-import-suite'), $term ) );
					...
					if ( strstr( $raw_term, '>' ) ) {
						...
						foreach ( $raw_term as $term ) {
							...
							if ( isset( $this->inserted_terms[ $taxonomy ][ $parent ][ $term ] ) ) {
								...
								$WC_CSV_Product_Import->log->add( 'CSV-Import', sprintf( __( '> > (' . __LINE__ . ') Term %s (%s) exists? %s', 'woocommerce-product-csv-import-suite' ), sanitize_text_field( $term ), esc_html( $taxonomy ), $term_may_exist ? print_r( $term_may_exist, true ) : '-' ) );
								...
								if ( ! $term_id ) {
									...
									if ( ! is_wp_error( $t ) ) {
										...
										$WC_CSV_Product_Import->log->add( 'CSV-Import', sprintf( __( '> > (' . __LINE__ . ') Failed to import term %s, parent %s - %s', 'woocommerce-product-csv-import-suite' ), sanitize_text_field( $term ), sanitize_text_field( $parent ), sanitize_text_field( $taxonomy ) ) );
										break;
									}
								}
						...
						if ( isset( $this->inserted_terms[ $taxonomy ][0][$raw_term] ) ) {
							...
							if ( ! $term_id ) {
								...
								if ( ! is_wp_error( $t ) ) {
									...
									$WC_CSV_Product_Import->log->add( 'csv-import', sprintf( __( '> > Failed to import term %s %s', 'woocommerce-product-csv-import-suite' ), esc_html($raw_term), esc_html($taxonomy) ) );
									break;
								}
							}
			...
			elseif ( strstr( $key, 'attribute:' ) ) {
				...
				if ( substr( $attribute_key, 0, 3 ) == 'pa_' ) {
					...
					if ( ! taxonomy_exists( $taxonomy ) ) {
						...
						$WC_CSV_Product_Import->log->add( 'csv-import', sprintf( __('> > Attribute taxonomy "%s" does not exist. Adding it. Nicename: %s', 'woocommerce-product-csv-import-suite'), $taxonomy, $nicename ) );
						...
						if ( ! $exists_in_db ) {
							...
							$WC_CSV_Product_Import->log->add( 'csv-import', sprintf( __('> > Attribute taxonomy %s already exists in DB.', 'woocommerce-product-csv-import-suite'), $taxonomy ) );
						}
						...
						foreach ( $raw_terms as $raw_term ) {
							...
							if ( ! $term_id ) {
								...
								if ( ! is_wp_error( $t ) ) {
									...
									$WC_CSV_Product_Import->log->add( 'csv-import', sprintf( __( '> > Inserted Raw Term %s ID = %s', 'woocommerce-product-csv-import-suite' ), esc_html( $raw_term ), $term_id ) );
								} else {
									$WC_CSV_Product_Import->log->add( 'csv-import', sprintf( __( '> > Failed to import term %s %s', 'woocommerce-product-csv-import-suite' ), esc_html($raw_term), esc_html($taxonomy) ) );
									break;
								}
							} else {
								$WC_CSV_Product_Import->log->add( 'csv-import', sprintf( __( '> > Raw Term %s ID = %s', 'woocommerce-product-csv-import-suite' ), esc_html( $raw_term ), $term_id ) );
							}
