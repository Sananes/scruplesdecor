...
class WC_Additional_Variation_Images_Admin {
	...
	public function load_admin_scripts() {
		...
		if ( 'product' === get_post_type() ) {
			wp_enqueue_script( 'wc_additional_variation_images_admin_script', plugins_url( 'assets/js/admin-settings' . $suffix . '.js' , dirname( __FILE__ ) ), array( 'jquery' ) );
			...
			$localized_vars = array(
				...
				'adminAddImagesTip'         => __( 'Click on link below to add additional images. Click on image itself to remove the image. Click and drag image to re-order the image position.', 'woocommerce-additional-variation-images' ),
				'is23'                      => version_compare( WC_VERSION, '2.3.0', '>=' ) ? true : false
			);
			...
			if ( version_compare( WC_VERSION, '2.3.0', '>=' ) ) {
				wp_enqueue_style( 'wc_additional_variation_images_admin_style', plugins_url( 'assets/css/admin23.css', dirname( __FILE__ ) ) );
			} else {
				wp_enqueue_style( 'wc_additional_variation_images_admin_style', plugins_url( 'assets/css/admin.css', dirname( __FILE__ ) ) );
			}
		}
	...
	public function load_images_ajax() {
		...
		if ( 0 < count( $variation_ids ) ) {
			foreach( $variation_ids as $id ) {
				...
				$html = '';
				$html .= '<input type="hidden" class="wc-additional-variations-images-thumbs-save" name="wc-additional-variations-images-thumbs[' . esc_attr( $id ) . ']" value="' . esc_attr( $ids ) . '">';
		...
		echo json_encode( array( 'images' => $variation_images ) );
		exit;
	}
	...
	public function wc_additional_variation_image_save( $post_id, $post ) {
		...
		if ( empty( $post_id ) || empty( $post ) || ! isset( $_POST['wc-additional-variations-images-thumbs'] ) ) {
			return;
		}
		...
		$ids = $_POST['wc-additional-variations-images-thumbs'];
