...
(function ( $ ) {
	...
	var VCTeaser = Backbone.View.extend( {
		...
		render: function () {
			...
			if ( ! _.isEmpty( this.$bgcolor.val() ) ) {
				this.$list.css( 'backgroundColor', this.$bgcolor.val() );
			}
			this.$spinner.show();
		...
		renderButtons: function ( controls ) {
			_.each( controls, function ( button ) {
				var html = _.template( $( '#vc_teaser-button' ).html(),
					{
						label: window.i18nVcTeaser[ button + '_label' ],
						value: button
					},
					template_options );
		...
		createControl: function ( data ) {
			...
			var html = _.template( $( '#vc_teaser-' + data.name ).html(),
				_.extend( default_data, data ),
				template_options );
		...
		showMediaEditor: function ( e ) {
			e.preventDefault();
			window.wp.media.VcTeaserCustomImage.frame( $( '.vc_teaser_add_custom_image',
				this.$el ).get( 0 ) ).open( 'vc_editor' );
		},
		...
		setCustomTeaserImage: function ( selection ) {
			if ( _.isObject( selection ) ) {
				this.custom_image_attributes = selection.attributes;
			}
		...
		appendCustomImageHtml: function () {
			this.$spinner.hide();
			$( '.vc_teaser-custom-image-view', this.$el ).html( _.template( $( '#vc_teaser-custom-image' ).html(),
				this.custom_image_attributes ) );
		},
		...
		updateTitle: function ( e ) {
			...
			if ( _.isEmpty( title ) ) {
				title = '<span class="vc_empty">' + window.i18nVcTeaser.empty_title + '</span>';
			}
		...
		save: function () {
			...
			$( '.vc_teaser-control', this.$el ).each( function () {
				...
				if ( $block.find( '.vc_link-controls' ).length ) {
					...
					if ( $active.length ) {
						block_data.link = $active.data( 'link' );
					}
				}
